...
public class SignTest {
    static double alpha = 0.005; //significant level
	static int N = 40; // the number of pairs must be greater than N to perform sign test
    ...
    boolean sign_test(){
        if (Npairs < N) { // the number of pairs must be at least N to perform the test
           return false;
            ...
            double sn_cdf = standard_normal_cdf(x);
            if (1-sn_cdf <= alpha) {
                return true;
            } else
                return false;
